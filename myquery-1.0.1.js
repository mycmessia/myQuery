...
function getByClass(oParent, sClass, sTag)
{
	...
	var oReg=new RegExp("\\b"+sClass+"\\b");
	var aResult=[];
...
function nodeList2arr(nodeList)
{
	...
	{
		aResult.push(nodeList[i]);
	}
	
	return aResult;
}

function selElement(arr, index, _this)
{
	var aResult=[];
	
	switch(arr[index].charAt(0))
	{
		case "#":
			aResult.push(document.getElementById(arr[index].substring(1)));
			break;
			...
			if(_this.elements.length)
			{
				for(var i=0;i<_this.elements.length;i++)
				{
					aResult=aResult.concat(
						getByClass(_this.elements[i],arr[index].substring(1))
					);
				}
			}
			else
			{
				aResult=getByClass(document,arr[index].substring(1));
			}
			break;
		...
		default:
			if(_this.elements.length)
			{
				...
				{
					aResult=aResult.concat(
						nodeList2arr(_this.elements[i].getElementsByTagName(arr[index]))
					);
				}
			}
			else
			{
				nl=document.getElementsByTagName(arr[index]);
				
				aResult=nodeList2arr(nl);
			}
			break;
	}
